...
void printDevProp() {
	...
	for (int i = 0; i<devCount; ++i) {
		...
		for (int axis = 0; axis < 3; ++axis)
			printf("Maximum dimension %d of block:  %d\n", axis, devProp.maxThreadsDim[axis]);
		for (int axis = 0; axis < 3; ++axis)
			printf("Maximum dimension %d of grid:   %d\n", axis, devProp.maxGridSize[axis]);
		int clockRate = 0, kernelExecTimeout = 0;
		cudaDeviceGetAttribute(&clockRate, cudaDevAttrClockRate, i);
		cudaDeviceGetAttribute(&kernelExecTimeout, cudaDevAttrKernelExecTimeout, i);
		printf("Clock rate:                    %d\n", clockRate);
		printf("Total constant memory:         %zu\n", devProp.totalConstMem);
		printf("Texture alignment:             %zu\n", devProp.textureAlignment);
		printf("Concurrent copy and execution: %s\n", (devProp.asyncEngineCount > 0 ? "Yes" : "No"));
