...
bool sampleFeatureValue(int imageIndex, vec3 parentPosition, out float featureValue)
{
    ...
    if (desc.flags.y != 0u) {
        ...
        intensityTex = texture(volume, position).r;
    }
    else {
    ...
    float x = intensityTex, y = 0.0;
    sampler2D featureTexture = sampler2D(desc.featureHandle.xy);
    if (desc.featureDims.y == 1.0)
        x = (intensityTex - desc.pixelParams.x) / max(desc.pixelParams.y - desc.pixelParams.x, 1e-6);
    else {
        offset_feature_texture(intensityTex, desc.featureDims.x, desc.featureDims.y, x, y);
...
void main()
{
    ...
    vec3 ray_start = current_ray_origin + current_ray_direction * t_0;
    vec3 ray_stop = current_ray_origin + current_ray_direction * t_1;
    ...
    a_colour = vec4(0.0);
    for (int curImage = 0; curImage < nbImages; ++curImage)
        a_colour += computeImageColor(images[curImage], intensity[curImage]);
