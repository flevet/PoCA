...
namespace voronator {
	Voronator::Voronator(delaunator::DelaunayFromDelaunator& _delau,
		...
		for (; i < _delau.delaunator.triangles.size(); i += 3, j += 2) {
			...
			double b[] = { x2, y2 };
			double c[] = { x3, y3 };
			const double ad = a[0] * a[0] + a[1] * a[1];
			const double bd = b[0] * b[0] + b[1] * b[1];
			const double cd = c[0] * c[0] + c[1] * c[1];
		...
		for (std::size_t n = 0; n < ncells; n++) {
			...
			x1 = delaunator.coords[n * 2];
			y1 = delaunator.coords[n * 2 + 1];
			//std::cout << "For cell " << n << " [" << x1 << ", " << y1 << "] : " << neighs_cell.size() << " - " << (sizes[n]/2) << std::endl;
			//std::cout << "\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\" << std::endl;
			//std::cout << "For locs " << n << " [" << x1 << ", " << y1 << std::endl;
			if (neighs_cell.size() != (size / 2))
				std::cout << "Unexpected" << std::endl;
			for (std::size_t i = 0; i < neighs_cell.size(); i++) {
				std::size_t index = neighs_cell[i];
				if (index == std::numeric_limits < size_t >::max()) continue;
				const double x2 = delaunator.coords[index * 2];
				const double y2 = delaunator.coords[index * 2 + 1];
				//std::cout << "Neighbor locs " << index << " [" << x2 << ", " << y2 << std::endl;
			}
		}
	}
	...
	void Voronator::neighbors(std::size_t i, std::vector <size_t> & neighs, std::size_t nbToAchieve) {
		neighs.clear();
		size_t sizeI = firsts[i + 1] - firsts[i];
		if (sizeI == 0) return;

		double * ci = cells.data() + firsts[i];
		std::vector <std::size_t> delau_neighs;
		delaunay.neighbors(i, delau_neighs);

		for (std::size_t jt = 0; jt < delau_neighs.size(); jt++) {
			std::size_t j = delau_neighs[jt];
			size_t sizeJ = firsts[j + 1] - firsts[j];
			if (sizeJ == 0) continue;
			double * cj = cells.data() + firsts[j];

			bool done = false;
			std::size_t li = sizeI, lj = sizeJ;
			for (std::size_t ai = 0; ai < li && !done; ai += 2) {
				for (std::size_t aj = 0; aj < lj && !done; aj += 2) {
					if (ci[ai] == cj[aj]
						&& ci[ai + 1] == cj[aj + 1]
		...
		for(std::size_t n = neighs.size(); n < nbToAchieve; n++)
			neighs.push_back(std::numeric_limits < size_t >::max());
	}
