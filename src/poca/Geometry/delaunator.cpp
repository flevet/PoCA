...
DelaunayFromDelaunator::DelaunayFromDelaunator(Delaunator & _delau) :delaunator(_delau), points(_delau.coords) {
	...
	std::fill(inedges.begin(), inedges.end(), -1);
	std::fill(hull_index.begin(), hull_index.end(), -1);
	...
	for (std::size_t e = 0, n = delaunator.halfedges.size(); e < n; ++e) {
		...
		if (delaunator.halfedges[e] == -1 || inedges[p] == -1) inedges[p] = e;
	}
...
std::size_t DelaunayFromDelaunator::step(std::size_t i, double x, double y) {
	if (inedges[i] == -1 || !points.size()) return (i + 1) % (points.size() >> 1);
	std::size_t c = i;
...
void DelaunayFromDelaunator::trianglesAdjacentToTriangle(std::size_t i, std::vector <std::size_t>& neighs)
{
    ...
    for (size_t e : edgesOfTriangle) {
        ...
        /*if (neighs[cur - 1] != INVALID_INDEX && ((neighs[cur - 1] % 3) != 0)) {
            std::cout << "Problem with tri " << i << " and neigh " << neighs[cur - 1] << ", opposite was " << opposite << std::endl;
        }*/
        //if (opposite == INVALID_INDEX)
        //    std::cout << INVALID_INDEX << " - " << std::numeric_limits<std::size_t>::max() << " " << neighs[cur - 1] << std::endl;
    }
}
