...
struct ConvexCell {
	...
	__device__ ConvexCell(int p_seed, float* p_pts, Status *p_status, float * p_bbox) {
		/*float w = p_bbox[3] - p_bbox[0], h = p_bbox[4] - p_bbox[1], t = p_bbox[5] - p_bbox[2];
		float maxD = w < h ? w : h;
		maxD = maxD < t ? maxD : t;*/
		float eps = 0.f;// maxD / 100.f;
		...
		*status = success;
		...
		/*clip(0) = make_float4(xmax, 0.0, 0.0, 1);
		clip(1) = make_float4(xmin, 0.0, 0.0, 1);
		clip(2) = make_float4(0.0, ymax, 0.0, 1);
		clip(3) = make_float4(0.0, ymin, 0.0, 1);
		clip(4) = make_float4(0.0, 0.0, zmax, 1);
		clip(5) = make_float4(0.0, 0.0, zmin, 1);*/
		nb_v = 6;
...
//###################  KERNEL   ######################
__device__ void compute_voro_cell_first_ring(float * pts, int nbpts, unsigned int* ci, unsigned int* ro, Status * gpu_stat, float* volumes, float * bbox, /*float * _cells, unsigned int * _nbTCells, */int seed) {
	//create BBox
	volumes[seed] = -1;
	...
	//export_volume(cc, volumes, seed, _cells, _nbTCells);
	float4 tet_bary;
	float tet_vol;
	float4 bary_sum = make_float4(0, 0, 0, 0);
	float cell_vol = 0;
...
//----------------------------------KERNEL
__global__ void voro_cell_test_GPU_param_first_ring(float * pts, int nbpts, unsigned int* ci, unsigned int* ro, Status * gpu_stat, float* _volumes, float * bbox) {
...
//###################  KERNEL   ######################
__device__ void compute_voro_cell_first_ring_with_construction_cells(float * pts, int nbpts, unsigned int* ci, unsigned int* ro, Status* gpu_stat, float* volumes, float * bbox, float * _cells, unsigned int * _nbTCells, int seed, unsigned int numPointsChunkIndex) {
	//create BBox
	ConvexCell cc(seed, pts, &(gpu_stat[seed]), bbox);
...
//----------------------------------KERNEL
__global__ void voro_cell_test_GPU_param_first_ring_with_construction_cells(float * pts, int nbpts, unsigned int* ci, unsigned int* ro, Status* gpu_stat, float* _volumes, float * bbox, float * _cells, unsigned int * _nbTCells, unsigned int _numPointChunkIndex, unsigned int _chunkNbPoints) {
...
void computeVoronoiFirstRing(std::vector <float> & _pts, std::vector <unsigned int> & _ci, std::vector <unsigned int> & _ro, std::vector <float> & _volumes, std::vector <float> & _cells, std::vector <unsigned int> & _nbTCells, bool _no_ConstructCells, unsigned int _sizeBuffer)
{
	//printDevProp();
	...
	//FOR(n, 3) bbox[n] = floor(bbox[n]);
	//FOR(n, 3) bbox[n+3] = ceil(bbox[n+3]);

	GPUBuffer<float> out_pts(_pts);
	GPUBuffer<unsigned int> out_ci(_ci);
	GPUBuffer<unsigned int> out_ro(_ro);
	...
	std::cout << "# points = " << nb_pts << ", size cell buffer = " << _cells.size() << std::endl;

	if (_no_ConstructCells){
		//GPUBuffer<float> out_cells(_cells);
		...
		for (unsigned int n = 0; n < numChunks; n++){
			...
			//std::cout << "From point " << numPointsChunkIndex << " to " << (numPointsChunkIndex + numPoints) << std::endl;
			//std::cout << "From buffer " << chunkIndex << " to " << (chunkIndex + bufferLength) << std::endl;
