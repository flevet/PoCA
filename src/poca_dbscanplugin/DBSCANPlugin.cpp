...
std::vector <std::pair<QAction*, QString>> DBSCANPlugin::getActions()
{
	return m_actions;
}

poca::core::MyObjectInterface* DBSCANPlugin::actionTriggered(QObject* _sender, poca::core::MyObjectInterface* _obj)
{
	return NULL;
}

void DBSCANPlugin::addCommands(poca::core::CommandableObject* _bc)
{
	poca::geometry::DetectionSet* dset = dynamic_cast <poca::geometry::DetectionSet*>(_bc);
	if (dset) {
		dset->addCommand(new DBSCANCommand(dset));
	}
}
