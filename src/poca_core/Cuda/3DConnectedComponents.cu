...
poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const T* _pixels, const T _thresholdMin, const T _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d) {
    ...
    JustGPUBuffer<T> imageToThreshold(_pixels, nbValues);
    kernel_threshold << <grid, block >> > (imageToThreshold.gpu_data, _thresholdMin, _thresholdMax, thresholImage, nbValues);

    thrust::device_vector<uint32_t> d_labels(nbValues);
    connectedComponnetsLabelingBinary(thresholImage, nbValues, _w, _h, _d, thrust::raw_pointer_cast(d_labels.data()));
    cudaFree(thresholImage);
    relabel_kernel_gpu<uint32_t>(d_labels);
    uint32_t maxLabel = *thrust::max_element(d_labels.begin(), d_labels.end());
    std::cout << "Max lbl = " << maxLabel << std::endl;
...
template poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const uint8_t* _pixels, const uint8_t _thresholdMin, const uint8_t _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d);
template poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const uint16_t* _pixels, const uint16_t _thresholdMin, const uint16_t _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d);
template poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const uint32_t* _pixels, const uint32_t _thresholdMin, const uint32_t _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d);
template poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const int32_t* _pixels, const int32_t _thresholdMin, const int32_t _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d);
template poca::core::ImageInterface* connectedComponnetsLabeling3DGPU(const float* _pixels, const float _thresholdMin, const float _thresholdMax, const uint32_t _w, const uint32_t _h, const uint32_t _d);
#endif