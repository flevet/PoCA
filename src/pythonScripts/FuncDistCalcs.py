...
#        print('Input data has fewer points (' + str(inputdata2.shape[0]) + ') than the furthest requested distance measurement (' + str(ps['FurthestFriend']) + ')')
        maxNeighbours = inputdata2.shape[0] - 1 # excluding self
        padNeighbours = ps['FurthestFriend'] - inputdata2.shape[0] + 1
        TestRegionSize = ps['ImageSize'][0]
#        UseableRegion = True
#        TestRegion = inputdata2
    else:
        # there are more points available than neighbours required
        maxNeighbours = ps['FurthestFriend']
        padNeighbours = 0
        # Guesstimate a starting region size based on the density and assuming the points are only in about 1/3 of the available area
        TestRegionSize = np.ceil(np.sqrt(maxNeighbours / (inputdata2.shape[0] / ( 0.3 * ps['ImageSize'][0] * ps['ImageSize'][1] ))))
#        UseableRegion = False
    ...
    while not UseableRegion:
        ...
        if np.shape(TestRegion)[0] >= maxNeighbours + 1:
            UseableRegion = True
            # Have a useable region but enlarge it by >= sqrt(2) to make sure
    ...
    dists = cdist(inputdata1[:, [ps['xCol'], ps['yCol']]],TestRegion[:, [ps['xCol'], ps['yCol']]]).T

    # join the distances and the neighbouring points' data
    ...
    if padNeighbours > 0:
        # add the missing NN distances as nans.
